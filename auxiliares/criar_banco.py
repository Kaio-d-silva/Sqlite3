...
cursor.execute('''CREATE TABLE contas_a_receber (id INTEGER PRIMARY KEY AUTOINCREMENT,valor_a_pagar text, data_lancamento text,data_vencimento text, observacao text, status boolean, id_cliente, foreign key (id_cliente) references cliente(id) on delete cascade )''')
